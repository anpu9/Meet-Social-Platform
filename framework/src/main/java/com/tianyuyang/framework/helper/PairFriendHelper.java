...
import com.tianyuyang.framework.bmob.FateSet;
import com.tianyuyang.framework.bmob.IMUser;
import com.tianyuyang.framework.utils.CommonUtils;
...
public class PairFriendHelper {
    ...
    private void loveUser(List<IMUser> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (imUser.getObjectId().equals(meUserId)) { //过滤自己
                //跳过本次循环
                continue;
            }
            if (imUser.isSex() != meIMUser.isSex()) { //异性
    ...
    private void fateUser() {
        ...
        BmobManager.getInstance().addFateSet(new SaveListener<String>() {
            ...
            public void done(final String s, BmobException e) {
                if (e == null) {
                    //轮询
                    mDisposable = Observable.interval(1, TimeUnit.SECONDS).subscribe(new Consumer<Long>() {
                        @Override
    ...
    private void rxJavaParingResult(final OnRxJavaResultListener listener) {
        //延迟
        mDisposable = Observable.timer(DELAY_TIME, TimeUnit.SECONDS).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<Long>() {
            @Override
            public void accept(Long aLong) throws Exception {
    ...
    List<String> objectIdList = list.parallelStream().filter(imUser -> !imUser.getObjectId().equals(meUserId)).filter(imUser -> imUser.getConstellation().equals(meIMUser.getConstellation())).filter(imUser -> imUser.getAge() == meIMUser.getAge() || imUser.getHobby().equals(meIMUser.getHobby()) || imUser.getStatus().equals(meIMUser.getStatus())).map(IMUser::getObjectId).collect(Collectors.toList());

    IMUser imUser = list.stream().skip((int) (list.size() * Math.random())).findFirst().orElse(null);
if(imUser!=null){
        onPairResultListener.OnPairListener(imUser.getObjectId());
        }
...
=
        if(FateNumber>=15){ //超时
            ...
            FateNumber=0;}
        return;}
        ListIterator<FateSet> iterator=list.listIterator();
        while(iterator.hasNext()){
            FateSet fateSet=iterator.next();
            if(fateSet.getUserId().equals(meUserId)){
            iterator.remove();
            break;}
        }
        int _r=mRandom.nextInt(list.size()); //最终结果
        onPairResultListener.OnPairListener(list.get(_r).getUserId());
        deleteFateSet(id); //删除自己
        FateNumber=0;

        
        for (IMUser user : list) {
            if (!user.getObjectId().equals(meUserId) && user.isSex() != meIMUser.isSex()) {
            potentialPartners.add(user);}}
        if (potentialPartners.isEmpty()) {// 查看是否有潜在的好友
            onPairResultListener.OnPairFailListener();
            return;
        }
        List<IMUser> ageFilteredPartners = new ArrayList<>();
        for (IMUser user : potentialPartners) { // 通过年龄筛选好友
            if (Math.abs(user.getAge() - meIMUser.getAge()) <= 3) {
                ageFilteredPartners.add(user);}}
